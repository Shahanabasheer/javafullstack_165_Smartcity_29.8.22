...
#updateCollege_div1 legend {
	color: white;
}

#updateCollege_div1 label {
	font-size: 15px;
	color: white;
	width: 170px;
	clear: left;
...
#updateCollege_div1 input {
	font-size: 15px;
	color: black;
	width: auto;
}

#updateCollege_div1 fieldset {
...
#updateCollege_div2 ul {
	list-style: none;
}

#updateCollege_div2 input[type="radio"] {
	margin-left: 10px;
}

#updateCollege_div2 ul li {
	display: inline-block;
}

#updateCollege_div3 ul {
	list-style: none;
}

#updateCollege_div3 input[type="radio"] {
	margin-left: 10px;
}

#updateCollege_div3 ul li {
	display: inline-block;
}

#updateCollege_div4 ul li {
	display: inline-block;
}

#updateCollege_div4 input[type="checkbox"] {
	margin-left: 10px;
}

#updateCollege_div5 select {
	color: black;
}

#updateCollege_div6 textarea {
	margin-left: 140px;
	border-radius: 10px;
	color: black;
}

#updateCollege_div6 input[type="submit"] {
	width: 120px;
	margin-left: 65px;
...
<body id="body_Updatecollege">
	...
	<div class="update_college" id="update_college">
		<h3>
			<strong>Update College</strong>
		</h3>
		...
		<form action="update_College.jsp?College_Id=<%=c_id%>" method="post" name="updateCollegeFM"
			id="updateCollegeFM" onsubmit="return formValidation()">
			<div id="updateCollege_div1">
				<fieldset>
					<legend>Basic Information</legend>

					<label id="cname">Name of college:</label><input type="text"
						name="collegeName" id="collegeName"
						value="<%=c.getCollegeName()%>">&nbsp<span id="e_1"></span><br>
					<label>Place:</label> <input type="text" name="collegePlace"
						id="collegePlace" value="<%=c.getCollegePlace()%>"><span
						id="e_2"></span><br> <label>Affiliation</label><input
						type="text" name="collegeAffiliation" id="collegeAffiliation"
						value="<%=c.getCollegeAffiliation()%>"><span id="e_3"></span><br>
					<label>College Type :</label>
					<div id="updateCollege_div2">
						<ul>
					...
					<div id="updateCollege_div6">
						<label>College Address</label><br>
						<textarea name="collegeAddress" id="collegeAddress" rows="4"
							cols="30"><%=c.getCollegeAddress()%></textarea>
						&nbsp <span id="e_4"></span> <br> <br> <label>Email</label><input
							type="email" name="collegeEmail" id="collegeEmail"
							value="<%=c.getCollegeEmail()%>">&nbsp<span id="e_5"></span><br>
						<label>College Website </label><input type="url"
							name="collegeWebsite" id="collegeWebsite"
							value="<%=c.getCollegeWebsite()%>"><span id="e_6"></span><br>
						<label>Contact Number</label><input type="tel" name="collegePhone"
							id="collegePhone" value="<%=c.getCollegePhone()%>"><span
							id="e_7"></span><br> <label>Principal Name</label><input
							type="text" name="collegePrincipal" id="collegePrincipal"
							value="<%=c.getCollegePrincipal()%>"><span id="e_8"></span><br>
						<label>About college</label><br>
						<textarea name="collegeAbout" id="collegeAbout" rows="6" cols="30"><%=c.getCollegeAbout()%></textarea>
						<br> <label>Upload college logo</label><input type="file"
							name="collegeLogo" id="collegeLogo"><br> <input
							type="submit" name="collegeUpdate" id="collegeUpdate"
							value="Update">
					</div>
				</fieldset>
			</div>
