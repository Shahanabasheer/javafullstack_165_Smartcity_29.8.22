...
<style type="text/css">
#body_Updateschool {
	background-image: url("images/bg_AdminEdu.jpg");
...
#updateSchool_div1 legend {
	color: white;
}

#updateSchool_div1 label {
	font-size: 15px;
	color: white;
	width: 170px;
	clear: left;
...
#updateSchool_div1 input {
	font-size: 15px;
	color: black;
	width: auto;
}

#updateSchool_div1 input, label {
	
}

#updateSchool_div1 fieldset {
...
#updateSchool_div2 ul {
	list-style: none;
}

#updateSchool_div2 input[type="radio"] {
	margin-left: 10px;
}

#updateSchool_div2 ul li {
	display: inline-block;
}

#updateSchool_div3 ul {
	list-style: none;
}

#updateSchool_div3 input[type="radio"] {
	margin-left: 10px;
}

#updateSchool_div3 ul li {
	display: inline-block;
}

#updateSchool_div4 ul li {
	display: inline-block;
}

#updateSchool_div4 input[type="checkbox"] {
	margin-left: 10px;
}

#updateSchool_div5 select {
	color: black;
}

#updateSchool_div6 textarea {
	margin-left: 140px;
	color: black;
}

#updateSchool_div6 input[type="submit"] {
	width: 120px;
	margin-left: 65px;
...
<script type="text/javascript">
	function formValidation() {
		...
		var s_affiliation = document.getElementById('schoolAffiliationNum').value;
		let aff = 0;
		var re = /^\d{6}$/;
		...
		var school_updateress = document.getElementById('schoolupdateress').value;
		let sad = 0;
		var re = /^[a-zA-Z,'.\s-]+$/;

		if (re.test(school_updateress)) {
			e_8.textContent = "Valid updateress";
			e_8.style.color = "white";
			sad = 1;
		} else {
			e_8.textContent = "** Not a valid updateress !";
			e_8.style.color = "white";
			document.getElementById('schoolupdateress').focus();
			sad = 0;
		}

		if (sn == 1 && sp == 1 && aff == 1 && em == 1 && ph == 1 && web == 1
				&& pri == 1 && sad == 1) {
			alert("Valid details and School updateed successfully");
			return true;
		} else {
...
<body id="body_Updateschool">
	...
	<div class="update_school" id="update_school">
		<h3>
		...
		<form action="update_School.jsp?School_Id=<%=s_id%>" method="post"
			name="updateSchoolFM" id="updateSchoolFM"
			onsubmit="return formValidation()">
			<div id="updateSchool_div1">
				<fieldset>
					<legend>Basic Information</legend>

					<label id="sname">Name of school:</label> <input type="text"
						name="schoolName" id="schoolName" value="<%=s.getSchoolName()%>">&nbsp<span
						id="e_1"></span><br> <label>Place:</label> <input type="text"
						name="schoolPlace" id="schoolPlace" value=<%=s.getSchoolPlace()%>>&nbsp<span
						id="e_2"></span><br> <label>Affiliation Number:</label><input
						type="text" name="schoolAffiliationNum" id="schoolAffiliationNum"
						value="<%=s.getSchoolAffiliationNum()%>"><span id="e_3"></span><br>
					<span id="error_3"></span> <label>School Type :</label>
					<div id="updateSchool_div2">
						<ul>
					...
					<div id="updateSchool_div6">
						<label>School address</label><br>
						<textarea name="schoolAddress" id="schoolAddress" rows="4"
							cols="30"><%=s.getSchoolAddress()%></textarea>
						<span id="e_8"></span> <br> <label>Email</label><input
							type="email" name="schoolEmail" id="schoolEmail"
							value="<%=s.getSchoolEmail()%>"> <span id="e_4"></span><br>
						<label>School Website </label><input type="url"
							name="schoolWebsite" id="schoolWebsite"
							value="<%=s.getSchoolWebsite()%>"><span id="e_6"></span><br>
						<label>Contact Number</label><input type="tel" name="schoolPhone"
							id="schoolPhone" value="<%=s.getSchoolPhone()%>"> <span
							id="e_5"></span><br> <label>Principal Name</label><input
							type="text" name="schoolPrincipal" id="schoolPrincipal"
							value="<%=s.getSchoolPrincipal()%>"><span id="e_7"></span><br>
						<label>About school</label><br>
						<textarea name="schoolAbout" id="schoolAbout" rows="6" cols="30"><%=s.getSchoolAbout()%></textarea>
						<br> <label>Upload school logo</label><input type="file"
							name="schoolLogo" id="schoolLogo"><br> <input
							type="submit" name="shcoolupdate" id="schoolupdate"
							value="Update">
